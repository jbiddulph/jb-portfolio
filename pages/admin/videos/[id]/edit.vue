<template>
  <div>
    <div class="mb-6 sm:mb-8">
      <NuxtLink to="/admin/videos" class="text-indigo-600 hover:text-indigo-500 mb-3 sm:mb-4 inline-block text-sm sm:text-base">
        ← Back to Videos
      </NuxtLink>
      <h1 class="text-2xl sm:text-3xl font-bold text-gray-900">Edit Video</h1>
      <p class="mt-1 sm:mt-2 text-sm sm:text-base text-gray-600">Update your video showcase details</p>
    </div>
...
const updateVideo = async () => {
  ...
  try {
    ...
    alert('Error updating video: ' + (error.message || 'Unknown error'))
  } finally {
    loading.value = false
  }
}
